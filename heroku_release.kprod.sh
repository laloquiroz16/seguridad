...
HEROKU_APP_KPROD=$(echo $HEROKU_APP_KPROD)
...
if [ -z "$HEROKU_APP_KPROD" ]; then
    echo "No HEROKU_APP_KPROD set"
    exit 1
fi 
...
blob_urls=$(curl_heroku_body "POST" "/apps/$HEROKU_APP_KPROD/slugs" '{"process_types":{"web":"nodejs/bin/node server.js"}}')
...
curl_heroku_body "POST" "/apps/$HEROKU_APP_KPROD/releases" $build_body
